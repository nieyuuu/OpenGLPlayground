...
int main()
{
	...
	unsigned int skyboxTexture = generateTextureCube(skyboxFaces);

	CShader cubeShader("cube_vertex.vs", "cube_fragment.fs");
	CShader skyboxShader("skybox_vertex.vs", "skybox_fragment.fs");
	...
	while (!glfwWindowShouldClose(sWindow))
	{
		...
		cubeShader.setTexture2DUniformValue("Texture", cubeTexture, 0);
		...
		skyboxShader.active();
		skyboxShader.setMat4UniformValue("View", glm::mat4(glm::mat3(View)));
		skyboxShader.setMat4UniformValue("Projection", Projection);
		skyboxShader.setTexture2DUniformValue("Skybox", skyboxTexture, 0);
